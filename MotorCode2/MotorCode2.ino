...
void loop() {
  ...
  if (Serial.available() > 0) {
    ...
    num_steps = MoveInt/step_size;
    if (MoveInt > 0) {  
      //for (int i = 0; i < num_steps; i++)
      //{      
      //  myMotor->step(step_size, FORWARD, SINGLE);
      //  delay(2000);
      //}
      myMotor->step(MoveInt,FORWARD,SINGLE); 
    } 
    else {
      //for (int i = 0; i < -num_steps; i++)
      //{      
      //  myMotor->step(step_size, BACKWARD, SINGLE);
      //  delay(2000);
      //}
      myMotor->step(- MoveInt,BACKWARD,SINGLE);
    } 
    myMotor->release();
  //}//else
       /* if(MoveInt < 5 ){
